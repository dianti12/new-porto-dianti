...
if (isset($_POST['simpan'])) {
    ...
    $tmp_name = $_FILES['photo']['tmp_name']; //yang baru belum ada file foto
    ...
    if (mysqli_num_rows($queryProfile) > 0) {
        ...
        if (!empty($photo)) {  //jika foto tidak kosong
            ...
            unlink("uploads/" . $rowProfile['photo']);          // untuk replace gambar
            move_uploaded_file($tmp_name, $filePath);

            // ketika berhasil mengambil gambar, larinya ke upload

            $update = mysqli_query($config, "UPDATE profiles SET profile_name='$profile_name', description='$description', photo='$fileName' WHERE id = '$id' ");
